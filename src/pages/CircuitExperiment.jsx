...
export default function CircuitExperiment() {
  ...
  const StageComponents = [CircuitIntro, CircuitLab, CircuitData, CircuitConclusion];
    ...
    <div className="h-screen flex flex-col bg-background font-body overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-sm font-bold font-heading leading-none">Simple Electric Circuits</h1>
                <p className="text-xs text-muted-foreground">Physics · Series &amp; Parallel</p>
              </div>
            </div>
          </div>
          ...
          {STAGES.map((s, i) => (
            <button key={s.id} onClick={() => goTo(i)} className="flex-1 flex flex-col items-center gap-1">
              <div className={`w-full h-1.5 rounded-full transition-all duration-300 ${
                i < step ? "bg-amber-500" : i === step ? "bg-amber-400" : "bg-muted"
              }`} />
              <span className={`text-[9px] font-medium hidden sm:block transition-colors ${
                i === step ? "text-amber-600" : "text-muted-foreground/50"
              }`}>{s.label}</span>
            </button>
          ))}
        </div>
      ...
      <footer className="shrink-0 bg-background/90 backdrop-blur-xl border-t border-border px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <Button variant="outline" onClick={back} disabled={step === 0} className="gap-2 min-w-[90px]">
          ...
          <div className="flex items-center gap-1.5">
            {STAGES.map((_, i) => (
              <motion.div key={i} onClick={() => goTo(i)}
                animate={{
                  width: i === step ? 20 : 8,
                  backgroundColor: i === step ? "hsl(38,92%,50%)" : "hsl(220,14%,85%)",
                }}
          ...
          {step < STAGES.length - 1 ? (
            <Button onClick={next} className="gap-2 min-w-[90px] bg-amber-500 hover:bg-amber-600 text-white border-0">
              Next <ArrowRight className="w-4 h-4" />
            </Button>
          ) : (
            <Button onClick={reset} className="gap-2 min-w-[90px] bg-amber-500 hover:bg-amber-600 text-white border-0">
