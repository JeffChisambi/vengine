...
export default function RefractionExperiment() {
  ...
  const StageComponents = [RefractionIntro, RefractionLab, RefractionData, RefractionConclusion];
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-sky-500/10 flex items-center justify-center">
                <svg viewBox="0 0 24 24" className="w-4 h-4 text-sky-600" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 3v18M3 12h18" strokeLinecap="round" />
                  <path d="M5 7l7 5 7-5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                <h1 className="text-sm font-bold font-heading leading-none">Refraction Through Glass</h1>
                <p className="text-xs text-muted-foreground">Physics · Optics · Snell's Law</p>
              </div>
            </div>
          </div>
          <Button
          ...
          {STAGES.map((s, i) => (
            <button key={s.id} onClick={() => goTo(i)} className="flex-1 flex flex-col items-center gap-1">
              <div className={`w-full h-1.5 rounded-full transition-all duration-300 ${
                i < step ? "bg-sky-500" : i === step ? "bg-sky-400" : "bg-muted"
              }`} />
      ...
      <footer className="shrink-0 bg-background/90 backdrop-blur-xl border-t border-border px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          {step < STAGES.length - 1 ? (
            <Button
              onClick={next}
              className="gap-2 min-w-[90px] bg-sky-600 hover:bg-sky-700 text-white border-0"
            >
