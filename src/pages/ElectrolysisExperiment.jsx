...
import {
  ...
  Zap, CheckCircle2, Microscope, BarChart3, BookOpen,
} from "lucide-react";
...
const STEPS = [
  ...
  { label: "Electrolysis", tag: "Step 2",     icon: Zap },
  { label: "Observations", tag: "Step 3",     icon: Microscope },
  { label: "Equations",    tag: "Step 4",     icon: BookOpen },
  { label: "Applications", tag: "Step 5",     icon: BarChart3 },
...
export default function ElectrolysisExperiment() {
  ...
  const [timeElapsed, setTimeElapsed]   = useState(0); // 0–1
  const [voltage, setVoltage]           = useState(6);
  const [hasRun, setHasRun]             = useState(false);
  const timerRef = useRef(null);

  /* Current derived from Ohm's Law (simple model) */
  const current = running ? (voltage / 12) * 0.8 : 0;

  /* Step 4 — equations reveal */
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Chemistry · Electrochemistry</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        },
                        {
                          ...
                          color: "text-cyan-700", bg: "bg-cyan-500/8", border: "border-cyan-500/20",
                          dot: "bg-cyan-500",
                        },
                      ].map((item, i) => (
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5">
            {STEPS.map((_, i) => (
              <motion.div key={i} onClick={() => goTo(i)}
                animate={{
                  width: i === step ? 20 : 8,
                  backgroundColor: i === step ? "hsl(186,85%,38%)" : "hsl(220,14%,85%)",
                }}
                transition={{ duration: 0.3 }}
          ...
          {step < TOTAL_STEPS - 1 ? (
            <Button onClick={handleNext} disabled={!canAdvance}
              className="gap-2 min-w-[90px] bg-cyan-600 hover:bg-cyan-700 text-white border-0"
              title={!canAdvance ? "Complete this step first" : ""}>
              Next <ArrowRight className="w-4 h-4" />
            </Button>
          ) : (
            <Button onClick={handleReset} className="gap-2 min-w-[90px] bg-cyan-600 hover:bg-cyan-700 text-white border-0">
              <RotateCcw className="w-4 h-4" /> Restart
            </Button>
          )}
        </div>
