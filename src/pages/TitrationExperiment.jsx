...
const STEPS = [
  { id: 0, label: "Intro" },
  { id: 1, label: "Apparatus" },
  { id: 2, label: "Indicator" },
  { id: 3, label: "Titration" },
  { id: 4, label: "Endpoint" },
  { id: 5, label: "Molecules" },
  { id: 6, label: "pH Data" },
...
export default function TitrationExperiment() {
  ...
  const addDrop = useCallback((size = "coarse") => {
    setVolumeAdded((prev) => {
      ...
      const next = Math.min(prev + amount, 28);
      const newNeutr = next / V_ENDPOINT;
      const newFlash = newNeutr > 0.8 ? Math.min(1, (newNeutr - 0.8) * 5) : 0.3;
  ...
  return (
    <ExperimentShell
      ...
      subject="Chemistry · Quantitative Analysis"
      icon={FlaskConical}
      theme={THEME}
      stages={STEPS}
      step={step}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button key={ind.id}
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">{ind.change}</p>
              ...
              {overshot && (
                <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
                  <p className="text-xs text-amber-700 font-medium">
                    ...
                    giving a concentration slightly above the true value.
                  </p>
                </div>
              )}
            </div>
