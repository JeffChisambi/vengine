...
const W = 480, H = 380, CX = 240, CY = 190, ATOM_R = 110;
...
export default function StageThomson({ onNext }) {
  ...
  const startFiring = () => {
    ...
    setParticles([]); setAnswered(null); setShowHint(false);
    fireParticle();
    setTimeout(() => { clearTimeout(timerRef.current); setShowHint(true); }, 6000);
  };
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full items-center justify-center">
        ...
        <div className="relative rounded-2xl border border-white/10 bg-[#0d1f17] overflow-hidden">
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="max-w-full">
            <ThomsonAtom />
            {particles.map((p) => (
              <motion.g key={p.id} initial={{ x: -20 }} animate={{ x: W + 20 }} transition={{ duration: 1.1, ease: "linear" }}>
                <line x1={-30} y1={p.startY} x2={0} y2={p.startY} stroke="#fbbf24" strokeOpacity={0.25} strokeWidth={1} />
                <circle cx={0} cy={p.startY} r={5} fill="#fbbf24" fillOpacity={0.9} />
