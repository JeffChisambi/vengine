...
const FACTS = [
  { value: "99.9999999999%", label: "of an atom is empty space",   color: "text-green-400",  border: "border-green-500/20",  bg: "bg-green-500/8"  },
  { value: "100,000×",       label: "larger — atom vs nucleus",    color: "text-violet-400", border: "border-violet-500/20", bg: "bg-violet-500/8" },
  { value: "~10⁻¹⁵ m",      label: "diameter of the nucleus",     color: "text-amber-400",  border: "border-amber-500/20",  bg: "bg-amber-500/8"  },
  { value: "~10⁻¹⁰ m",      label: "diameter of the atom",        color: "text-rose-400",   border: "border-rose-500/20",   bg: "bg-rose-500/8"   },
];
    ...
    <svg viewBox="0 0 360 200" className="w-full max-w-sm mx-auto">
      <defs>
        <radialGradient id="atom-outer" cx="50%" cy="50%">
          ...
          <stop offset="80%"  stopColor="#4ade80" stopOpacity="0.03" />
      ...
      {/* Atom boundary — brand green */}
      <circle cx="180" cy="100" r="90" fill="url(#atom-outer)" stroke="#4ade80" strokeOpacity="0.2" strokeWidth="1" strokeDasharray="4 4" />
      <text x="180" y="12" textAnchor="middle" fontSize="9" fill="#4ade80" fillOpacity="0.5" fontFamily="monospace">← atom (~10⁻¹⁰ m) →</text>

      {/* Electrons — brand green */}
      {[{cx:110,cy:55},{cx:260,cy:80},{cx:140,cy:170},{cx:230,cy:155},{cx:95,cy:135},{cx:270,cy:135}].map((e, i) => (
        <motion.circle key={i} cx={e.cx} cy={e.cy} r={3} fill="#4ade80" filter="url(#nuc-glow2)"
  ...
  return (
    <div className="min-h-full flex flex-col gap-6 px-4 py-8 max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center">
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 text-green-400 text-xs font-semibold mb-3 border border-green-500/20">
          🌌 Stage 4 — Empty Space
        </span>
        <h2 className="text-2xl sm:text-3xl font-extrabold font-heading mb-2 text-white">Atoms Are Mostly Nothing</h2>
      ...
      {onNext && revealed.length === FACTS.length && (
        <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={onNext}
          className="self-center px-6 py-2.5 rounded-xl bg-green-700 hover:bg-green-600 text-white text-sm font-semibold transition-colors shadow-lg shadow-green-900/40"
        >
          Continue to Summary →
        </motion.button>
      )}
