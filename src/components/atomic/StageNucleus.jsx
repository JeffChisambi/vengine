  ...
  return (
    <g>
      ...
      {[0, 120, 240].map((deg, i) => {
        ...
        return (
          <motion.circle key={i} r={3.5} fill="#0ea5e9" fillOpacity={0.8}
            animate={{
  ...
  return (
    <div className="min-h-full flex flex-col gap-5 items-center justify-center px-4 py-8 max-w-5xl mx-auto">
      <div className="text-center">
        ...
        <h2 className="text-2xl sm:text-3xl font-extrabold font-heading mb-2">A Tiny, Dense, Powerful Core</h2>
        <p className="text-muted-foreground text-sm">Rutherford concluded a dense positive nucleus must exist. Watch how it deflects particles.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 w-full items-center justify-center">
        ...
        <div className="rounded-2xl border border-border bg-slate-50 overflow-hidden">
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="max-w-full">
            ...
            <ellipse cx={CX} cy={CY} rx={160} ry={60} fill="none" stroke="#0ea5e9" strokeOpacity={0.15} strokeWidth={1} />
            {showPaths.map((p) => {
            ...
            <polygon points={`6,${CY} 26,${CY - 8} 26,${CY + 8}`} fill="#94a3b8" fillOpacity={0.5} />
            <text x={CX + 12} y={CY - 14} fontSize={9} fill="#fbbf24" fillOpacity={0.9} fontFamily="var(--font-heading)">Nucleus</text>
          </svg>
        </div>
        ...
        <div className="flex flex-col gap-4 w-full sm:max-w-[220px]">
          ...
          <div className="p-3 rounded-xl bg-muted/40 border border-border space-y-3">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Nucleus Charge</p>
            <input type="range" min={1} max={6} value={charge} onChange={(e) => setCharge(+e.target.value)} className="w-full accent-sky-500" />
            <div className="flex justify-between text-[10px] text-muted-foreground">
              <span>Weak</span>
              <span className="text-sky-600 font-bold">×{charge}</span>
              <span>Strong</span>
            </div>
          </div>

          <div className="p-3 rounded-xl bg-muted/40 border border-border space-y-2 text-xs">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Path Colors</p>
          ...
          {fired && showPaths.length > 5 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-3 rounded-xl bg-sky-500/10 border border-sky-500/25 text-xs space-y-1">
              <p className="text-sky-700 font-semibold">💡 Key insight</p>
              <p className="text-muted-foreground">The nucleus must be <em>very tiny</em> — most particles miss it entirely. But when one gets close, the intense positive charge repels it sharply.</p>
            </motion.div>
          )}
        </div>
      </div>
