...
// Compute curved repulsion path toward the nucleus
function buildRepulsionPath(b) {
  ...
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    const x = t * W;
    // Approach CX from left
    const dx = x - CX;
    const dy = CY + b;
    const dist = Math.sqrt(dx * dx + (dy - CY) * (dy - CY));
    // Repulsion deflection: larger when closer
    ...
    pts.push([
      ...
      CY +
        b +
        (x > CX - 100 && x < CX + 80
          ? repulsion * Math.exp(-Math.abs(dx) / 60)
          : 0),
    ]);
  }
  return pts;
      ...
      {[0, 120, 240].map((deg, i) => {
        ...
        return (
          <motion.circle
            ...
            animate={{
              cx: [
                CX + 160 * Math.cos(angle),
                CX + 160 * Math.cos(angle + Math.PI),
                CX + 160 * Math.cos(angle + 2 * Math.PI),
              ],
              cy: [
                CY + 60 * Math.sin(angle),
                CY + 60 * Math.sin(angle + Math.PI),
                CY + 60 * Math.sin(angle + 2 * Math.PI),
              ],
            }}
  ...
  return (
    <div className="min-h-full flex flex-col gap-5 items-center justify-center px-4 py-8 max-w-5xl mx-auto">
      <div className="text-center">
        <span className="inline-block px-3 py-1 rounded-full bg-amber-500/15 text-amber-400 text-xs font-semibold mb-3">
          Stage 3 — The Nucleus
        </span>
        <h2 className="text-2xl sm:text-3xl font-extrabold font-heading text-white mb-2">
          A Tiny, Dense, Powerful Core
        </h2>
        <p className="text-white/50 text-sm">
          Rutherford concluded a dense positive nucleus must exist. Watch how it
          deflects particles.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 w-full items-center justify-center">
        {/* Simulation */}
        <div className="rounded-2xl border border-white/10 bg-[#060c18] overflow-hidden">
          <svg
            width={W}
            height={H}
            viewBox={`0 0 ${W} ${H}`}
            ...
            {showPaths.map((p) => {
              ...
              return (
                <motion.path
                  ...
                  fill="none"
                  stroke={color}
                  strokeWidth={Math.abs(p.b) < 30 ? 1.8 : 1}
                  strokeOpacity={Math.abs(p.b) < 30 ? 0.7 : 0.35}
        ...
        <div className="flex flex-col gap-4 min-w-[200px] w-full max-w-[220px]">
          <Button
            onClick={fire}
            disabled={animating}
            className="bg-amber-600 hover:bg-amber-500 text-white border-0 rounded-xl text-sm"
          >
            ⚡ Fire Particles
          </Button>
          ...
          <div className="p-3 rounded-xl bg-white/4 border border-white/8 space-y-3">
            ...
            <div className="flex justify-between text-[10px] text-white/40">
              <span>Weak</span>
              <span className="text-amber-300 font-bold">×{charge}</span>
              <span>Strong</span>
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-white/4 border border-white/8 space-y-2 text-xs">
            ...
            {[
              ["#fbbf24", "Far — straight through"],
              ["#f43f5e", "Near — deflected"],
              ["#e879f9", "Close — back-scattered"],
            ].map(([c, l]) => (
          ...
          {fired && showPaths.length > 5 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/25 text-xs space-y-1"
            >
              <p className="text-amber-300 font-semibold">💡 Key insight</p>
              <p className="text-white/60">
                The nucleus must be <em>very tiny</em> — most particles miss it
                entirely. But when one gets close, the intense positive charge
                repels it sharply.
              </p>
            </motion.div>
          )}
