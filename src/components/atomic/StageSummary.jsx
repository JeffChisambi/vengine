  ...
  return (
    <div className="min-h-full flex flex-col gap-7 items-center justify-center px-4 py-10 max-w-3xl mx-auto">
      <div className="text-center">
        <span className="inline-block px-3 py-1 rounded-full bg-emerald-500/15 text-emerald-400 text-xs font-semibold mb-3">
        ...
        <h2 className="text-2xl sm:text-3xl font-extrabold font-heading text-white mb-2">
          {allDone
            ? "You've made Rutherford's discovery!"
            : "What did Rutherford discover?"}
        </h2>
        <p className="text-white/50 text-sm">
          Tap each question to reveal the conclusion you discovered.
        </p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all ${open ? `${c.border} ${c.color}` : "border-white/20"}`}
                >
                  {open ? (
                ...
                <div className="flex-1">
                  <p
                    className={`text-sm font-semibold ${open ? c.color : "text-white/70"}`}
                  >
                    {c.q}
                  </p>
                  <AnimatePresence>
      ...
      >
        {FACTS.map((f, i) => (
          ...
          >
            <p className="text-base font-extrabold font-heading text-cyan-400">
              {f.value}
            </p>
      ...
      <AnimatePresence>
        {allDone && (
          <motion.div
            ...
            className="w-full p-5 rounded-2xl bg-gradient-to-br from-cyan-500/10 to-violet-500/10 border border-cyan-500/20 text-center"
          >
            <p className="text-lg font-extrabold font-heading text-white mb-1">
              🏆 Rutherford's Nuclear Model
            </p>
            <p className="text-sm text-white/60 leading-relaxed">
              The atom is mostly empty space with a tiny dense nucleus at its
              center. Electrons orbit at huge distances. This model replaced
              Thomson's "plum pudding" — and it came from a simple experiment
              with gold foil.
            </p>
          </motion.div>
        )}
