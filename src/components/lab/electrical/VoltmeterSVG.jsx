...
export default function VoltmeterSVG({
  voltage = 5.2,
  measuring = true,
  glow = false,
}) {
  const [val, setVal] = useState(voltage);
  const t = useRef(null);
  useEffect(() => {
    if (!measuring) return;
    const tick = () => {
      setVal(voltage + (Math.random() - 0.5) * 0.3);
  ...
  const needleAngle = (val / 10) * 130 - 65;
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id="vmFace" cx="50%" cy="45%" r="55%">
          <stop offset="0%" stopColor="#f8fafc" />
          <stop offset="100%" stopColor="#e2e8f0" />
        </radialGradient>
        <linearGradient id="vmCase" x1="0" y1="0" x2="1" y2="1">
      ...
      <circle cx="110" cy="148" r="2" fill="#16a34a" />
